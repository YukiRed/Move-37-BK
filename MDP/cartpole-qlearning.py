...
def play_one_game(bins, Q, eps=0.5, render=False):
    ...
    while not done:
        env.render() if render else False
        cnt += 1
...
def play_many_games(bins, N=10000):
    ...
    for n in range(N):
        ...
        episode_reward, episode_length = play_one_game(
            bins, Q, eps, render=True if n % 100 == 0 else False)
